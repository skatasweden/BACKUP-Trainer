-- Fix the get_exercise_with_context function to use correct program_item video settings
CREATE OR REPLACE FUNCTION public.get_exercise_with_context(plan_item_id_param uuid, exercise_id_param uuid DEFAULT NULL::uuid, protocol_id_param uuid DEFAULT NULL::uuid)
 RETURNS json
 LANGUAGE plpgsql
...
BEGIN
  ...
  -- This ensures we get the right video settings for the right program_item
  ...
  ORDER BY pa.created_at DESC -- Use the most recently accessed program
  LIMIT 1;
