-- Update get_workout_with_details to accept sort_order_param for precise program_item filtering
CREATE OR REPLACE FUNCTION public.get_workout_with_details(
  ...
  sort_order_param integer DEFAULT NULL::integer
)
 RETURNS json
 LANGUAGE plpgsql
...
begin
  ...
  if program_id_param is not null then
    ...
    debug_info := 'Using provided program_id: ' || program_id_param::text || ', sort_order: ' || coalesce(sort_order_param::text, 'NULL');
  else
  ...
  left join lateral (
    -- Get program_item from the specific program context - PRIORITIZE BY EXACT MATCH
    ...
    from (
      ...
      order by 
        -- CRITICAL: Exact match on both program_id AND sort_order gets highest priority
        case when pi.program_id = target_program_id and sort_order_param is not null and pi.sort_order = sort_order_param then 0 else 1 end,
        case when pi.program_id = target_program_id then 0 else 1 end,
        -- Prioritize items with video data
        case when pi.show_video = true or pi.video_url is not null then 0 else 1 end,
        pi.updated_at desc,
        pi.created_at desc,
        pi.id desc
      limit 1
